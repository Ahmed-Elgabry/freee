...
if ($f == 'register') {
    ...
    if (empty($_POST['email']) || empty($_POST['username']) || empty($_POST['password']) || empty($_POST['confirm_password']) || empty($_POST['gender']) || empty($_POST['birthday'])) {
    ...
    if (empty($errors)) {
        ...
        if ($register === true) {
            ...
            if ($activate == 1) {
                ...
                $data    = array(
                    'status' => 301,
                    'location' => Wo_SeoLink('index.php?link1=welcome')
                );
