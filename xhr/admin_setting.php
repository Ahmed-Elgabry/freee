...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'delete_version') {
        if (!empty($_GET['version_id']) && is_numeric($_GET['version_id']) && $_GET['version_id'] > 0) {
            
            $query= mysqli_query($sqlConnect, "DELETE FROM " . T_VERSION_APP . " WHERE id='{$_GET['version_id']}'");
    ...
    if ($s == 'add_new_version') {
        $insert_data         = array();
        $insert_data['type'] = $_POST['type'];
        $insert_data['version_number'] = $_POST['version_number'];
        $insert_data['status'] = $_POST['status'];
        
        
        $id = $db->insert(T_VERSION_APP, $insert_data);
    ...
    if ($s == 'get_version') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            $data['status'] = 200;
            $query_one     = "SELECT * FROM " . T_VERSION_APP . " WHERE `id` = '{$_POST['id']}'";
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
            $data['data'] = mysqli_fetch_assoc($sql_query_one);
        }
        header("Content-type: application/json");
        echo json_encode($data);
        exit();
    ...
    if ($s == 'edit_verion') {
        $data['status'] = 200;
        if (!empty($_POST['edit_id']) && is_numeric($_POST['edit_id']) && $_POST['edit_id'] > 0) {

            $query = mysqli_query($sqlConnect, "UPDATE " . T_VERSION_APP . " SET type = '{$_POST['edit_type']}', version_number = '{$_POST['edit_version_number']}', status = '{$_POST['edit_status']}' WHERE id = {$_POST['edit_id']}");
            if($query)
            {
                $data['status'] = 200;
            }
