...
function Wo_check_AdminOrModerators($user_id = false, $page_id = false) {
    global $sqlConnect, $wo;
    $column_name = 'admins';
    $query = "SELECT `{$column_name}` FROM " . T_PAGE_ADMINS . " WHERE `user_id` = {$user_id} AND `page_id` = {$page_id}";
    $result = mysqli_query($sqlConnect, $query);

    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $column_value = $row[$column_name];
        if ($column_value == 1) echo $wo['lang']['admin'];
         else   echo $wo['lang']['moderators'];
    }
}
