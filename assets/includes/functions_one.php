...
function Wo_UserData2($user_id, $password = true)
{
    global $wo, $sqlConnect, $cache, $db;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
...
function Wo_GetSearch($search_qeury)
{
    ...
    $query = mysqli_query($sqlConnect, " SELECT `id` FROM " . T_POSTS . " WHERE `postText` LIKE '%$search_qeury%' AND `active` = '1' LIMIT 3");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = Wo_PostData($fetched_data['id']);
        }
    }
    $query = mysqli_query($sqlConnect, " SELECT `id` FROM " . T_JOB . " WHERE `title` LIKE '%$search_qeury%' AND `active` = '1' LIMIT 3");
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_GetSearchFilter2($result, $limit = 30, $offset = 0)
{
    global $wo, $sqlConnect, $db;
    $data = array();
    $profiledata = array();
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data[$fetched_data['user_id']] = Wo_UserData2($fetched_data['user_id']);
        }
    }
    // if( !empty( $profile_search ) ){
    //     $profile_sql_query_one = mysqli_query($sqlConnect, $profile_search_sql);
...
function Wo_GetReelsHome()
{
    global $sqlConnect;
    $random_offset = mt_rand(0, 1000);

    $query_text = "SELECT `id` FROM " . T_POSTS . " WHERE `is_reel` = 1 ORDER BY RAND(" . $random_offset . ") LIMIT 100";
