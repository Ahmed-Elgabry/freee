...
function Wo_ReportPost($post_data = [])
{
    ...
    $query_two = 'INSERT INTO '.T_REPORTS." (`user_id`, `post_id`, `comment_id`, `is_reel`, `time`) VALUES ({$user_id}, {$post_id}, {$comment_id}, {$is_reel}, ".time().')';
...
function Wo_GetAllPosts($posts = ['limit' => 10, 'after_user_id' => 0])
{
    global $wo, $sqlConnect;
    $data = [];
    $subquery = '';
...
function Wo_UpdateAdsCode($update_data = [])
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
...
function Wo_GetLikes($user_id = 0, $type = '', $limit = '', $after_user_id = '', $placement = [])
{
    global $wo, $sqlConnect;
    $data = [];
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
...
function Wo_GetUsersGroups($user_id = 0, $limit = 12, $placement = [], $offset = 0)
{
    global $wo, $sqlConnect;
    $data = [];
    if (empty($user_id) or !is_numeric($user_id) or $user_id < 1) {
...
function Wo_IsNameExist($username, $active = 0)
{
    ...
    ($query = mysqli_query($sqlConnect, 'SELECT COUNT(`id`) as usergroups,`id` as id FROM '.T_GROUPS." WHERE `group_name` = '{$username}' {$active_text}")) or exit(mysqli_error($sqlConnect));
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
...
function Wo_GetGroupRequestsWithOffset($request = [])
{
    global $wo, $sqlConnect;
...
function Wo_AlbumImageData($data = [])
{
    global $wo, $sqlConnect;
    if (!empty($data['id'])) {
...
function Wo_RegisterCommentReply($data = [])
{
...
function Wo_GetSearchAdv($search_qeury, $type, $offset = 0, $limit = 0)
{
    ...
    if ($type == 'groups') {
        ...
        $query = mysqli_query($sqlConnect, ' SELECT `id` FROM '.T_JOB." WHERE `title` LIKE '%$search_qeury%' ORDER BY `id` DESC LIMIT 10");
        if (mysqli_num_rows($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = Wo_GetJobById($fetched_data['id']);
            }
        }
    } /*elseif ($type == "ads") {
        $query = mysqli_query($sqlConnect, " SELECT `id` FROM " . T_USER_ADS . " WHERE `headline` LIKE '%$search_qeury%' ORDER BY `id` DESC LIMIT 10");
        if (mysqli_num_rows($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = Wo_GetUserAdData($fetched_data["id"]);
            }
        }
    }*/
...
function Wo_CreateNewAgoraCall($re_data = [])
{
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
...
{
    global $wo, $sqlConnect;
    $var = str_replace('64', '64_', str_replace('|', '', 'b|a|s|e|6|4|d|e|c|o|d|e'));
...
function Wo_DeleteProMemebership()
{
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            ...
            if ($update_data == true) {
                $update = Wo_UpdateUserData($fetched_data['user_id'], [
                    'is_pro' => 0,
                    'verified' => 0,
                    'pro_type' => 0,
                    'pro_' => 1,
...
function Wo_GetGenders($lang = 'english', $langs = [])
{
    ...
    $genders = $db->where('type', 'gender')->get(T_LANGS, null, [
        'lang_key',
        $lang,
    ]);
    $data = [];
        ...
        while ($fetched_data = mysqli_fetch_assoc($pro)) {
            ...
            if (!empty($fetched_data['time']) && $fetched_data['time'] == 'day') {
                if (!empty($fetched_data['time_count']) && is_numeric($fetched_data['time_count']) && $fetched_data['time_count'] > 0) {
                    $fetched_data['ex_time'] *= $fetched_data['time_count'];
                }
            } elseif (!empty($fetched_data['time']) && $fetched_data['time'] == 'week') {
                $fetched_data['ex_time'] *= 7;
                if (!empty($fetched_data['time_count']) && is_numeric($fetched_data['time_count']) && $fetched_data['time_count'] > 0) {
                    $fetched_data['ex_time'] *= $fetched_data['time_count'];
                }
            } elseif (!empty($fetched_data['time']) && $fetched_data['time'] == 'month') {
                $fetched_data['ex_time'] *= 30;
                if (!empty($fetched_data['time_count']) && is_numeric($fetched_data['time_count']) && $fetched_data['time_count'] > 0) {
                    $fetched_data['ex_time'] *= $fetched_data['time_count'];
                }
            } elseif (!empty($fetched_data['time']) && $fetched_data['time'] == 'year') {
                $fetched_data['ex_time'] *= 365;
                if (!empty($fetched_data['time_count']) && is_numeric($fetched_data['time_count']) && $fetched_data['time_count'] > 0) {
                    $fetched_data['ex_time'] *= $fetched_data['time_count'];
                }
            } elseif (!empty($fetched_data['time']) && $fetched_data['time'] == 'unlimited') {
        ...
        if ($result) {
            if ($result['data']) {
                if ($result['data']['status'] == 'success') {
                    ...
                    exit('Transaction was not successful: Last gateway response was: '.$result['data']['gateway_response']);
                }
            } else {
                exit($result['message']);
            }
        } else {
            exit('Something went wrong while trying to convert the request variable to json. Uncomment the print_r command to see what is in the result variable.');
        }
    } else {
        exit('Something went wrong while executing curl. Uncomment the var_dump line above this line to see what the issue is. Please check your CURL command to make sure everything is ok');
    }
}
...
function affiliateRef($price)
{
    ...
    if ($wo['config']['amount_percent_ref'] > 0) {
        if (!empty($ref_user_id) && is_numeric($ref_user_id)) {
            $update_user = Wo_UpdateUserData($wo['user']['user_id'], [
...
function Wo_DeleteVersion($id)
{
    global $sqlConnect;
    $id = Wo_Secure($id);

    $query_two = mysqli_query($sqlConnect, 'DELETE FROM '.T_VERSION_APP." WHERE `id` = {$id}");
    if ($query_two) {
        return true;
    }
}
