...
if($chat_type != 'user')
{
    ...
    {
        $profile_id = $list['user_id'];
        $reported = 0;
        $sql = " SELECT `id` FROM " . T_REPORTS . " WHERE `profile_id` = '{$profile_id}' AND `user_id` = '{$user_id}'";
        $data_rows = mysqli_query($sqlConnect, $sql);
        $match = mysqli_num_rows($data_rows);
        ...
        if($start < $end)
        {
            $data[$start]['user_id'] = $list['user_id'];
            $data[$start]['reported'] = $reported;
            $data[$start]['username'] = $list['username'];
            $data[$start]['first_name'] = $list['first_name'];
            $data[$start]['last_name'] = $list['last_name'];
            $data[$start]['avatar'] = $list['avatar'];
            $data[$start]['cover'] = $list['cover'];
            $data[$start]['lastseen'] = $list['lastseen'];
            $data[$start]['lastseen_status'] = $list['lastseen_status'];
            $data[$start]['showlastseen'] = $list['showlastseen'];
            $data[$start]['is_pro'] = $list['is_pro'];
            $data[$start]['is_verified'] = $list['is_verified'];
            $data[$start]['url'] = $list['url'];
            $data[$start]['name'] = $list['name'];
            $data[$start]['chat_id'] = $list['chat_id'];
            $data[$start]['chat_type'] = $list['chat_type'];
            $data[$start]['last_message'] = $list['last_message'];
            $data[$start]['mute'] = $list['mute'];
            $data[$start]['message_count'] = $list['message_count'];
            $start++;
        }
