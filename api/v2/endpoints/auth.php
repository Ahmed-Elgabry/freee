...
if (empty($error_code)) {
    ...
    elseif (Wo_IsBanned($username)) {
        ...
        if (!$login) {
            ...
            $error_code    = 8;
            $error_message = $wo['lang']['account_disbaled_contanct_admin_label'];
            ...
            if ($send) {
                $response_data = array(
                    'api_status' => 220,
                    'message' => 'Login successful! We have sent you an email, Please check your inbox/spam to verify your email.',
                    'user_id' => $user_id
                );
            } else {
                $error_code    = 11;
