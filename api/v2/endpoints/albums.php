        ...
        if (!empty($_POST['album_name']) && !empty($_FILES['postPhotos']['name']))
        {
            ...
            if (isset($_FILES['postPhotos']['name']))
            {
                $allowed = array(
                    ...
                    'jpg',
                    'jpeg,  '
                    ,'docx','zip','rar','pdf','doc','mp3','mp4','flv','wav','txt','mov','avi','webm','wav','mpeg'
                );
                for ($i = 0;$i < count($_FILES['postPhotos']['name']);$i++)
                {
                    ...
                    {
                        $error_code = 6;
                        $error_message = 'Image format is not supported, (jpg, png, gif, jpeg,
                        ,docx,zip,rar,pdf,doc,mp3,mp4,flv,wav,txt,mov,avi,webm,wa
                        ) are supported';
                    }
                }
            }
