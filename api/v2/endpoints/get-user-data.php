...
if (empty($error_code)) {
    ...
    if (empty($recipient_data)) {
		...
		if (!empty($data['user_data'])) {
	        ...
	        $follow_query = mysqli_query($sqlConnect, " SELECT COUNT(`id`) FROM " . T_FOLLOWERS . " WHERE `following_id` = {$_POST['user_id']} AND `follower_id` = {$wo['user']['user_id']}");
    
            $recipient_data['follow_sent'] = (Wo_Sql_Result($follow_query, 0) == 1) ? true : false;
