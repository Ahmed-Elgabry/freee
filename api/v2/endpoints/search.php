...
$get_users = Wo_GetSearchFilter2($options, $options['limit'], $user_offset);
...
foreach ($get_users as $key => $user) {
    ...
    $query = mysqli_query($sqlConnect, " SELECT COUNT(`id`) FROM " . T_FOLLOWERS . " WHERE `following_id` = {$user['user_id']} AND `follower_id` = {$wo['user']['user_id']}");
    
    $user['follow_sent'] = (Wo_Sql_Result($query, 0) == 1) ? true : false;
    $users[] = $user;
}
